...
elements = ['infrastructure-odn1','cust-odn1-voip','vm-odn1-system-logs','cust-odn1-portal','cust-odn1-pltest','cust-odn1-servicewell','cust-odn1-vetech']
...
def main():
    ...
    for stat in stats:
		...
		if (stat["ElementName"].split(" ")[1] in elements):
                    nstats.append(storeNagiosValues(nstats, stat))
		else:
		    pass
        else:
...
def storeNagiosValues(nstats, stat):
    ...
    if options.volume:
        #pstats = name+"-ReadIOs="+str(stat['ReadIOs']), name+"-WriteIOs="+str(stat['WriteIOs']) #] , name+"-TotalIOs="+str(stat['TotalIOs']), name+"-ReadHitIOs="+ str(stat['ReadHitIOs']), name+"-WriteHitIOs="+str(stat['WriteHitIOs'])
        pstats = "|"+str(stat['ReadIOs'])+"|"+str(stat['WriteIOs'])

    elif options.port or options.disk:
        pstats = name+"-ReadIOs="+str(stat['ReadIOs']), name+"-WriteIOs="+str(stat['WriteIOs']), name+"-TotalIOs="+str(stat['TotalIOs'])
    return str(pstats)
